...
function renderSwRegisterEntry(data) {
  return '<script>' + 'window.onload = function () {' + 'var script = document.createElement("script");' + 'var firstScript = document.getElementsByTagName("script")[0];' + 'script.type = "text/javascript";' + 'script.async = true;' + 'script.src = "' + data.publicPath + data.fileName + '?v=" + Date.now();' + 'firstScript.parentNode.insertBefore(script, firstScript);' + '};' + '</script>';
}
...
SwRegisterPlugin.prototype.apply = function (compiler) {
  ...
  var emitCallback = function emitCallback(compilation, callback) {
    ...
    Object.keys(compilation.assets).forEach(function (asset) {
      ...
      if (!isIn(asset, me.excludes) && (/\.html$/.test(asset) || isIn(asset, me.includes))) {
        ...
        var swRegisterEntryFileContent;
